...
const searchKeyword = ref(''); // Lưu từ khóa tìm kiếm
...
const filteredPosts = computed(() => {
    if (!searchKeyword.value.trim()) {
        return; // Hiển thị bài viết hiện tại nếu không có từ khóa
    }
...
const getAll = () => {
    ...
    axios.get(import.meta.env.VITE_BASE_API + `/Blog`)
        .then(function (response) {
            // handle success
            posts.value = response.data.slice(0, morePost);
...
<template>
    ...
    <div class="row">
        <div class="news" v-for="post in posts" :key="post.id" v-show="post.favorite">
